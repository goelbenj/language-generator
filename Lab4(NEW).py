...
def prune_ngram_counts(counts, prune_len):
        ...
        for key in counts:
                ...
                while (j < len(count_list) and i < prune_len):
                        #if(val[1].count(count_list[j]) > 1 and j < len(count_list) - 1 and count_list[j+1] == count_list[j] and i+1 == prune_len):
...
def gen_bot_list(ngram_model, seed, num_tokens = 0):
        ...
        if num_tokens == 0:
                ...
                condition = (len(output_stALWAYS_CAPITALIZEring) >= num_tokens or (not (tuple(curr) in ngram_model.keys())))
                while not condition:
                        output_string.append(utilities.gen_next_token(tuple(curr), ngram_model))
                        current_ngram = output_string[-(len(curr)):]
                        condition = (len(output_string) >= num_tokens or (not (tuple(curr) in ngram_model.keys())))
...
if __name__ == "__main__":
        fname = '308.txt'
        #print(parse_story(fname))
        token_list = parse_story(fname)
