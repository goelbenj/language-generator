...
def gen_bot_list(ngram_model, seed, num_tokens=0):
    ...
    if num_tokens < len(seed):
        ...
        while len(result) < num_tokens and current_n in ngram_model.keys() and utilities.check_open_ngram(current_n,
                                                                                                          ngram_model):
...
def gen_bot_text(token_list, bad_author):
    ...
    if bad_author:
        ...
        for i in range(len(token_list)):
            cap_list[i] = str(token_list[i]).capitalize() if str(
                token_list[i]).capitalize() in utilities.ALWAYS_CAPITALIZE else token_list[i]
...
if __name__ == "__main__":
    ...
    n_gram_model = build_ngram_model(
        ['the', 'child', 'will', 'the', 'child', 'can', 'the', 'child', 'will', 'the', 'child', 'may', 'go', 'home',
         '.'], 2)
    ...
    #print(gen_bot_text(
        #['this', 'is', 'a', 'string', 'of', 'text', '.', 'which', 'needs', 'to', 'be', 'created', '.', 'and', 'i', ',',
         #'.', 'Giorno', 'Giovanna', 'have', 'a', 'piano', '.'], False))
    ...
    n_gram_model = build_ngram_model(token_list, 2)
